...
from AnkiVectorMusic.config import ARQ_API_KEY
from AnkiVectorMusic.config import BOT_NAME as bn
from AnkiVectorMusic.config import DURATION_LIMIT
from AnkiVectorMusic.config import UPDATES_CHANNEL as updateschannel
from AnkiVectorMusic.config import que
from AnkiVectorMusic.function.admins import admins as a
from AnkiVectorMusic.helpers.admins import get_administrators
from AnkiVectorMusic.helpers.channelmusic import get_chat_id
from AnkiVectorMusic.helpers.errors import DurationLimitError
...
from AnkiVectorMusic.helpers.gets import get_file_name
    ...
    mar = InlineKeyboardMarkup(
        [
            [
                ...
                InlineKeyboardButton("⏸ Puse", "puse"),
...
async def settings(client, message):
    if message.chat.id in DISABLED_GROUPS:
        await message.reply("<b>Music Player is Disabled❌</b>")
        return    
    playing = None
        ...
        marr = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("⏹Stop", "leave"),
                    InlineKeyboardButton("⏸Puse", "puse"),
                    InlineKeyboardButton("▶️Resume", "resume"),
                    InlineKeyboardButton("⏭Skip", "skip"),
                ],
                [
                    InlineKeyboardButton("Playlist 📖", "playlist"),
            ...
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    await USER.send_message(
                        message.chat.id, "<b>I joined this group for playing music in VC ✳️</b>"
                    )
                    await lel.edit(
    ...
    if chat_id in callsmusic.active_chats:
        ...
        try:
           await callsmusic.set_stream(chat_id, file_path)
            ...
            message.reply("<b>Group Call is not connected or I can't join it</b>")
            return
        await message.reply_photo(
            photo="final.png",
            reply_markup=keyboard,
            caption="▶️ **Playing** here the song requested by {} via Anki Vector Music 😜".format(
                message.from_user.mention()
            ),
        )
        os.remove("final.png")
...
async def deezer(client: Client, message_: Message):
    ...
    if chat_id in callsmusic.active_chats:
        ...
        try:
            ...
            res.edit("<b>Group call is not connected of I can't join it</b>")
            return
...
async def lol_cb(b, cb):
    ...
    if cb.from_user.id != useer_id:
        ...
        return
    await cb.message.edit("<b>Hang On... Player Starting❇️</b>")
    x=int(x)
    try:
    ...
    if chat_id in callsmusic.active_chats:
        ...
        await b.send_photo(chat_id,
            photo="final.png",
            reply_markup=keyboard,
            caption=f"▶️ **Playing** here the song requested by {r_by.mention} via Anki Vector Music 😎",
        )
